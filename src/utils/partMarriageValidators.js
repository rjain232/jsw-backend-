...
const VALID_SHIFTS = ['All', 'A', 'B', 'C', 'A,B', 'A,C', 'B,C', 'A,B,C'];
...
function validatePartMarriageQuery(query) {
    const errors = [];
    const result = {};
    ...
    result.subStationId = toPositiveInt(query.subStationId, 'subStationId', errors);

    // --- shift (comma-separated allowed, e.g. "A,B") ---
    ...
    if (result.subStationId && !(result.clusterId && result.lineId && result.stationId)) {
        errors.push('subStationId requires clusterId, lineId and stationId');
    }
...
module.exports = { validatePartMarriageQuery };
